...
const InstallPrompt: React.FC = () => {
    ...
    useEffect(() => {
        ...
        setIsDismissed(hasUserDismissedPrompt);

        // Reset dismissed state after 30 days
        const lastDismissed = parseInt(localStorage.getItem('installPromptDismissedAt') || '0', 10);
        const now = Date.now();
        if (hasUserDismissedPrompt && (now - lastDismissed > 30 * 24 * 60 * 60 * 1000)) {
        ...
        const handleBeforeInstallPrompt = (e: BeforeInstallPromptEvent) => {
            ...
            setTimeout(() => {
                if (!isRunningStandalone && !hasUserDismissedPrompt) {
                    setShowPrompt(true);

...
const Filters: React.FC<FiltersProps> = ({ onFilterChange }) => {
    ...
    return (
        <div className="mb-6 bg-fog/70 rounded-lg p-4 shadow-lg border border-forest/70 backdrop-blur-sm">
            <div className="flex justify-between items-center mb-2">
                <button 
                    onClick={toggleFilters}
                    className="flex items-center text-gold hover:text-ethereal transition-colors duration-300"
                >
                ...
                {(filters.genres.length > 0 || filters.platforms.length > 0 || filters.ordering || filters.dates) && (
                    <button 
                        ...
                        className="text-tan hover:text-gold text-sm transition-colors duration-300"
                    >
                        Clear All
                    </button>
                )}
            ...
            {showFilters && (
                <div className="mt-4 space-y-6">
                    {/* Ordering */}
                    <div>
                        <h3 className="text-mist font-medium mb-2">Sort By</h3>
                        <select
                            value={filters.ordering}
                            onChange={(e) => handleFilterChange('ordering', e.target.value)}
                            className="w-full bg-twilight/70 text-tan rounded-md p-2 
                                     border border-forest hover:border-gold 
                    ...
                    {/* Release Date Ranges */}
                    <div>
                        <h3 className="text-mist font-medium mb-2">Release Period</h3>
                        <select
                            value={filters.dates}
                    ...
                    {/* Genres */}
                    <div>
                        <h3 className="text-mist font-medium mb-2">Genres</h3>
                        {loading ? (
                            <div className="flex flex-wrap gap-2 animate-pulse">
                                {[...Array(8)].map((_, index) => (
                                    <div key={index} className="h-8 w-20 bg-twilight rounded"></div>
                                ))}
                            </div>
                        ) : (
                                    ...
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-full text-sm transition-all duration-300 ${
                                            filters.genres.includes(genre.id.toString())
                                                ? 'bg-sage text-twilight shadow-md shadow-sage/30'
                                                : 'bg-twilight/70 text-tan hover:bg-twilight hover:text-gold border border-forest'
                                        }`}
                                    >
                                        {genre.name}
                                    </button>
                    ...
                    {/* Platforms */}
                    <div>
                        <h3 className="text-mist font-medium mb-2">Platforms</h3>
                        {loading ? (
                            <div className="flex flex-wrap gap-2 animate-pulse">
                                {[...Array(8)].map((_, index) => (
                                    <div key={index} className="h-8 w-24 bg-twilight rounded"></div>
                                ))}
                            </div>
                        ) : (
                            <div className="flex flex-wrap gap-2">
                                    ...
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-full text-sm transition-all duration-300 ${
                                            filters.platforms.includes(platform.id.toString())
                                                ? 'bg-gold text-twilight shadow-md shadow-gold/30'
                                                : 'bg-twilight/70 text-tan hover:bg-twilight hover:text-gold border border-forest'
                                        }`}
                                    >
                                        {platform.name}
